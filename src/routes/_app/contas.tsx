        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            {v.status === "pago" && (
              <div className="space-y-1.5">
                ...
                <Input
                  type="date"
                  value={v.data_pagamento ?? v.vencimento}
          ...
          <div className="space-y-3 p-3 rounded-lg bg-muted/40">
            <div className="flex items-center justify-between">
              ...
              <Switch
                checked={v.recorrente}
                onCheckedChange={(c) => setV({ ...v, recorrente: c })}
              />
            </div>
            {v.recorrente && (
              <div className="space-y-1.5">
...
function Contas() {
  ...
  const setMes = useCallback(
    (novo: string) =>
      navigate({ search: (prev: ContasSearch) => ({ ...prev, mes: novo }), replace: true }),
    [navigate],
  );
  ...
  return (
    <>
      <PageHeader
        title="Despesas"
        description="Controle de contas e gastos do consultório"
        actions={
