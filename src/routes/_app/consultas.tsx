...
function Consultas() {
  ...
  return (
    <>
      <PageHeader title="Consultas Previstas" description="Previsão simples de atendimentos futuros"
        actions={<ConsultaForm />}
      />

      <div className="grid gap-4 sm:grid-cols-3 mb-6">
        <StatCard label="Consultas hoje" value={String(hoje)} tone="primary" icon={<CalendarDays className="h-4 w-4" />} />
        <StatCard label="Consultas na semana" value={String(semana)} icon={<CalendarClock className="h-4 w-4" />} />
        <StatCard label="Valor previsto da semana" value={brl(valorSemana)} tone="success" icon={<CircleDollarSign className="h-4 w-4" />} />
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        ...
        <Button variant={showRealizadas ? "default" : "outline"} onClick={() => setShowRealizadas((s) => !s)}>
        ...
        <Table>
          ...
          <TableBody>
            ...
            {!list.isLoading && rows.length === 0 && (
              <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground py-12">Nenhuma consulta prevista.</TableCell></TableRow>
            )}
            {rows.map((c) => {
              const atrasada = !c.realizada && c.data_prevista < todayKey;
              return (
                <TableRow key={c.id} className={cn(c.realizada && "opacity-60")}>
                  <TableCell className={cn("text-muted-foreground", atrasada && "text-destructive")}>
                    {formatDateBR(c.data_prevista)}
                  </TableCell>
                  <TableCell className="font-medium">{c.paciente}</TableCell>
                  <TableCell className="text-muted-foreground text-sm max-w-[220px] truncate">{c.observacao || "—"}</TableCell>
                  <TableCell className="text-right">{Number(c.valor_estimado) > 0 ? brl(c.valor_estimado) : "—"}</TableCell>
                  <TableCell>
                    {c.realizada ? (
